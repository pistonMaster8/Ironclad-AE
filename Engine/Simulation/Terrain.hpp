...
inline constexpr int   kHFDivs   = 200;     // grid subdivisions (matches ground mesh)
inline constexpr float kHFSize   = 90.0f;   // half-extent in world units (±90)
inline constexpr int   kHFStride = kHFDivs + 1;

inline float gHeightField[kHFStride * kHFStride] = {};
inline bool  gHeightFieldActive = false;

inline float Height(float x, float z) {
    if (!gHeightFieldActive) return 0.f;

    float fx = (x + kHFSize) / (2.f * kHFSize) * kHFDivs;
    float fz = (z + kHFSize) / (2.f * kHFSize) * kHFDivs;
    if (fx < 0.f) fx = 0.f; else if (fx > kHFDivs) fx = (float)kHFDivs;
    if (fz < 0.f) fz = 0.f; else if (fz > kHFDivs) fz = (float)kHFDivs;

    int   x0 = (int)fx, z0 = (int)fz;
    int   x1 = (x0 < kHFDivs) ? x0 + 1 : x0;
    int   z1 = (z0 < kHFDivs) ? z0 + 1 : z0;
    float tx = fx - (float)x0, tz = fz - (float)z0;

    float h00 = gHeightField[z0 * kHFStride + x0];
    float h10 = gHeightField[z0 * kHFStride + x1];
    float h01 = gHeightField[z1 * kHFStride + x0];
    float h11 = gHeightField[z1 * kHFStride + x1];
