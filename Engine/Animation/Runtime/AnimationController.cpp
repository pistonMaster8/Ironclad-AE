...
AnimationLibrary AnimationLibrary::BuildSoldierLibrary(const Skeleton& skel) {
    ...
    lib.Add(MakeSoldierJogClip(skel));
...
void AnimationController::Update(f32 dt, const TransformComponent& transform,
    ...
    // ── 2. Clip selection ─────────────────────────────────────────────────────
    if (overrideClip) {
        // Editor mode: force a specific clip and bypass the state machine entirely.
        if (sampler.GetCurrentClip() != overrideClip) sampler.SetClip(overrideClip, 0.15f);
    } else {
