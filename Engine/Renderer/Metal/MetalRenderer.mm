...
//
// Active in Ironclad AE — the editor feeds scene.skinnedBoneRot each frame.
...
struct MetalRendererImpl {
    ...
    struct TerrainNode { float x, y, z; bool isCorner; };
    static constexpr int kMaxTerrainNodes = 64;
...
// |∇H| of the construction plane at (x,z), via central differences.
static inline float IDWGradMag(MetalRendererImpl& d, float x, float z) {
    const float e = 0.5f;
    float dhx = IDWHeight(x+e, z, d.terrainNodes, d.terrainNodeCount)
              - IDWHeight(x-e, z, d.terrainNodes, d.terrainNodeCount);
    float dhz = IDWHeight(x, z+e, d.terrainNodes, d.terrainNodeCount)
              - IDWHeight(x, z-e, d.terrainNodes, d.terrainNodeCount);
...
static void GenerateTerracedHeightfield(MetalRendererImpl& d, float step, float height,
                                        float angleDeg) {
    using Terrain::kHFDivs;
    using Terrain::kHFStride;
    const float size = Terrain::kHFSize;
    const int   N    = kHFDivs;
    ...
    for (int zi = 0; zi <= N; ++zi)
        for (int xi = 0; xi <= N; ++xi) {
            ...
            float H = IDWHeight(x, z, d.terrainNodes, d.terrainNodeCount);
            float g = (tanT > 0.f) ? IDWGradMag(d, x, z) : 0.f;
            Terrain::gHeightField[zi * kHFStride + xi] = TerraceRemapY(H, g, p, tanT);
        }
...
static void BuildTerracedGroundMesh(MetalRendererImpl& d, float step, float height,
    ...
    for (int cj = 0; cj < N; ++cj) {
        for (int ci = 0; ci < N; ++ci) {
            ...
            float h0 = IDWHeight(x0, z0, d.terrainNodes, d.terrainNodeCount);
            float h1 = IDWHeight(x1, z0, d.terrainNodes, d.terrainNodeCount);
            float h2 = IDWHeight(x1, z1, d.terrainNodes, d.terrainNodeCount);
            float h3 = IDWHeight(x0, z1, d.terrainNodes, d.terrainNodeCount);
            ...
            float ym = TerraceBand(IDWHeight((x0+x1)*0.5f, (z0+z1)*0.5f,
                                             d.terrainNodes, d.terrainNodeCount), p.step) * p.outH;
            PV a, e;
...
// Bilinearly samples the terrain heightfield mirror (201×201 over ±90). Zero-filled = flat.
constant int   kGrHFDivs   = 200;
constant float kGrHFSize   = 90.0f;
constant int   kGrHFStride = 201;
static float grTerrH(float x, float z, device const float* hf) {
    float fx = clamp((x + kGrHFSize) / (2.0f*kGrHFSize) * float(kGrHFDivs), 0.0f, float(kGrHFDivs));
    float fz = clamp((z + kGrHFSize) / (2.0f*kGrHFSize) * float(kGrHFDivs), 0.0f, float(kGrHFDivs));
    int   x0 = int(fx), z0 = int(fz);
    int   x1 = min(x0+1, kGrHFDivs), z1 = min(z0+1, kGrHFDivs);
    float tx = fx - float(x0), tz = fz - float(z0);
    float h00 = hf[z0*kGrHFStride + x0], h10 = hf[z0*kGrHFStride + x1];
    float h01 = hf[z1*kGrHFStride + x0], h11 = hf[z1*kGrHFStride + x1];
                        ...
                        constant float2&        grassDensityParams [[buffer(6)]])
{
    ...
    // Terrain height + normal (finite differences)
    float eps = 0.4f;
    float hC  = grTerrH(wx, wz, hf);
    float3 norm = normalize(float3(
        grTerrH(wx - eps, wz, hf) - grTerrH(wx + eps, wz, hf),
        2.0f * eps,
        grTerrH(wx, wz - eps, hf) - grTerrH(wx, wz + eps, hf)
    ));

    float slopeDot   = norm.y;
    float eastFacing = max(0.0f, norm.x);

    // Per-blade stable random
    ...
    // Downward-step proximity (long blades only). 3 radii × 4 cardinal directions =
    // 12 height lookups (down from 30). Detects any terrain drop of 0.4+ units.
    float edgeT = 0.0f;
    if (!cull && bv == 15) {
        constexpr float kDropThr  = 0.4f;
        constexpr float kRadii[3] = { 1.5f, 3.0f, 5.0f };
        float stepDist = 1e9f;
        for (int ri = 0; ri < 3; ++ri) {
            float r   = kRadii[ri];
            bool  hit = false;
            // +X, -X, +Z, -Z
            hit = (grTerrH(wx + r, wz,     hf) <= hC - kDropThr) ||
                  (grTerrH(wx - r, wz,     hf) <= hC - kDropThr) ||
                  (grTerrH(wx,     wz + r, hf) <= hC - kDropThr) ||
                  (grTerrH(wx,     wz - r, hf) <= hC - kDropThr);
...
vertex SkyVert skyVS(uint vid [[vertex_id]]) {
    ...
    float2 pos = float2((vid == 1) ? 3.0f : -1.0f,
                        (vid == 2) ? 3.0f : -1.0f);
    SkyVert out;
    out.position = float4(pos, 1.0f, 1.0f);  // z=1 → drawn at far plane depth
    out.ndc = pos;
    return out;
}
...
{
    // Unproject NDC position to a world-space view ray
    float4 wFar  = sky.invViewProj * float4(in.ndc, 1.0f, 1.0f);
    float4 wNear = sky.invViewProj * float4(in.ndc, 0.0f, 1.0f);
    float3 rayDir = normalize(wFar.xyz / wFar.w - wNear.xyz / wNear.w);
    ...
    d.grassUnifBuf = [d.device newBufferWithLength:2 * sizeof(GpuGrassUniforms)
                                           options:MTLResourceStorageModeShared];
    // Write static geometry params once at init; only colors are updated per-frame.
    {
    ...
    d.skyUniformBuf = [d.device newBufferWithLength:sizeof(SkyGpuUniforms)
                                            options:MTLResourceStorageModeShared];
    ...
    d.interactBuf = [d.device newBufferWithLength:sizeof(d.interactField)
                                          options:MTLResourceStorageModeShared];
    memset(d.interactBuf.contents, 0, sizeof(d.interactField));
    d.shellCtrlBuf   = [d.device newBufferWithLength:sizeof(ShellCtrlGpu) options:MTLResourceStorageModeShared];
    d.longDensityBuf = [d.device newBufferWithLength:sizeof(float) * 2     options:MTLResourceStorageModeShared];
    {
        std::vector<GpuVertex> v; std::vector<uint16_t> i;
        BuildSphereMesh(v, i);
    ...
    }
    {
        ...
        d.selRingVertBuf = [d.device newBufferWithLength:vSize options:MTLResourceStorageModeShared];
        d.selRingIdxBuf  = [d.device newBufferWithLength:iSize options:MTLResourceStorageModeShared];
    }
    {
        ...
        d.cursorRingVertBuf = [d.device newBufferWithLength:vSize options:MTLResourceStorageModeShared];
        d.cursorRingIdxBuf  = [d.device newBufferWithLength:iSize options:MTLResourceStorageModeShared];
    }

    {
        ...
        d.dbgRingVertBuf = [d.device newBufferWithLength:vSize options:MTLResourceStorageModeShared];
        d.dbgRingIdxBuf  = [d.device newBufferWithLength:iSize options:MTLResourceStorageModeShared];
    }
    {
        ...
        d.auraVertBuf = [d.device newBufferWithLength:vSize options:MTLResourceStorageModeShared];
        d.auraIdxBuf  = [d.device newBufferWithLength:iSize options:MTLResourceStorageModeShared];
    }

    d.cursorInstBuf = [d.device newBufferWithLength:sizeof(GpuInstanceData)
                                             options:MTLResourceStorageModeShared];
    d.shadowInstBuf = [d.device newBufferWithLength:sizeof(GpuInstanceData) * MetalRendererImpl::kMaxInstances
                                             options:MTLResourceStorageModeShared];
    d.explosionInstBuf = [d.device newBufferWithLength:sizeof(GpuInstanceData) * MetalRendererImpl::kMaxInstances
                                               options:MTLResourceStorageModeShared];
    ...
    {
        constexpr float kCornerExt = 80.0f;
        d.terrainNodes[0] = { -kCornerExt, 0.f, -kCornerExt, true };
        d.terrainNodes[1] = {  kCornerExt, 0.f, -kCornerExt, true };
        d.terrainNodes[2] = { -kCornerExt, 0.f,  kCornerExt, true };
        d.terrainNodes[3] = {  kCornerExt, 0.f,  kCornerExt, true };
        d.terrainNodeCount = 4;
        d.nodeInstBuf = [d.device newBufferWithLength:sizeof(GpuInstanceData) * MetalRendererImpl::kMaxTerrainNodes
                                             options:MTLResourceStorageModeShared];
        UpdateCPMesh(d);
    }
    ...
    // ─── Load Soldier + bone buffer ───────────────────────────────────────────
    {
        ...
        d.boneBuf = [d.device newBufferWithLength:boneBufSize options:MTLResourceStorageModeShared];
        d.charInstBuf = [d.device newBufferWithLength:RenderScene::kMaxShadowDiscs * sizeof(GpuInstanceData)
                                              options:MTLResourceStorageModeShared];
        d.dotInstBuf  = [d.device newBufferWithLength:RenderScene::kMaxShadowDiscs * sizeof(GpuInstanceData)
                                              options:MTLResourceStorageModeShared];
        constexpr int kRingSegs = 64;
        d.ringInstBuf = [d.device newBufferWithLength:kRingSegs * RenderScene::kMaxFollowRings * sizeof(GpuInstanceData)
                                              options:MTLResourceStorageModeShared];
    ...
    d.instanceBuf = [d.device newBufferWithLength:sizeof(GpuInstanceData) * MetalRendererImpl::kMaxInstances
                                          options:MTLResourceStorageModeShared];
    d.uniformBuf  = [d.device newBufferWithLength:sizeof(GpuFrameUniforms)
                                          options:MTLResourceStorageModeShared];
...
void MetalRenderer::RenderScene(const ::RenderScene& scene) {
    ...
    // ─── Update frame uniforms ─────────────────────────────────────────────
    {
        ...
        simd_float4x4 proj = {{
            ...
            simd_make_float4( 0,  0, zs * near, 0)
        }};
    ...
    if (d.psoSky && d.skyUniformBuf && scene.drawEnvironment) {
    ...
    if (d.grassUnifBuf) {
        auto* gp = (GpuGrassUniforms*)d.grassUnifBuf.contents;
        // Static geometry fields written once at init; only update colors here.
    ...
    for (uint32_t i = 0; i < scene.propCount && propInstCount < (uint32_t)MetalRendererImpl::kMaxInstances; ++i) {
        ...
        auto& inst = instData[propInstCount++];
        float s = p.scale;
        inst.model = {{
            simd_make_float4(s, 0, 0, 0),
            simd_make_float4(0, s, 0, 0),
    ...
    for (uint32_t i = 0; i < scene.projectileCount && projInstCount < (uint32_t)MetalRendererImpl::kMaxInstances; ++i) {
        ...
        auto& inst = instData[projInstCount++];
        float s = p.radius;
        inst.model = {{
            simd_make_float4(s, 0, 0, 0),
            simd_make_float4(0, s, 0, 0),
    ...
    if (d.soldier.loaded && d.psoSkinned && d.boneBuf &&
        ...
        for (uint32_t i = 0; i < discCount; ++i) {
            ...
            // Bone matrices: retarget the procedural humanoid pose onto this mesh when
            // the editor feeds it (scene.skinnedBoneRot); else fall back to glTF clips.
    ...
    // Uses BuildGroundMesh (samples Terrain::Height()) because terrain nodes are
    // not persisted — only the heightfield is saved.  Heights are correct because
    // the heightfield already stores post-terrace values.
    if (scene.requestMeshRebuild) {
        if (d.heightFieldBuf)
            memcpy([d.heightFieldBuf contents], Terrain::gHeightField,
                   sizeof(Terrain::gHeightField));
        std::vector<GpuVertex> gv; std::vector<uint16_t> gi;
        BuildGroundMesh(gv, gi);
        ...
        d.groundIndexBuf  = [d.device newBufferWithBytes:gi.data()
                                                  length:gi.size() * sizeof(uint16_t)
                                                 options:MTLResourceStorageModeShared];
        d.groundIndexCount = (uint32_t)gi.size();
        d.groundIndexType  = MTLIndexTypeUInt16;
        TerraceParams tp  = MakeTerraceParams(scene.erosionStep, scene.erosionHeight);
        d.terraceStepDrop = (tp.step > 0.f) ? tp.outH : 0.0f;
    }
    ...
    if (scene.dModeActive) {
        ...
        if (scene.requestGenerate) {
            GenerateTerracedHeightfield(d, scene.erosionStep, scene.erosionHeight,
                                        scene.erosionAngle);                       // physics
            std::vector<GpuVertex> gv; std::vector<uint32_t> gi;
            bool vertical = (scene.erosionAngle <= 0.f || scene.erosionAngle >= 89.f);
            if (vertical)                                                          // crisp 90° walls
                BuildTerracedGroundMesh(d, scene.erosionStep, scene.erosionHeight, gv, gi);
            else                                                                   // inclined ramps
                BuildRampedGroundMesh(d, scene.erosionStep, scene.erosionHeight,
                                      scene.erosionAngle, gv, gi);
            d.groundVertexBuf = [d.device newBufferWithBytes:gv.data()
                                                      length:gv.size() * sizeof(GpuVertex)
                                                     options:MTLResourceStorageModeShared];
            d.groundIndexBuf  = [d.device newBufferWithBytes:gi.data()
            ...
            // Mirror the heightfield to the GPU so procedural grass rides the terrain.
            if (d.heightFieldBuf)
                memcpy([d.heightFieldBuf contents], Terrain::gHeightField,
                       sizeof(Terrain::gHeightField));

            // World rise per band — drives long-grass step-down density/tip effects.
            TerraceParams tp  = MakeTerraceParams(scene.erosionStep, scene.erosionHeight);
            d.terraceStepDrop = (tp.step > 0.f) ? tp.outH : 0.0f;
        }

        // Drag start: ray-sphere distance in 3D — works for elevated nodes too.
        // Click on any node sphere regardless of its Y position.
        ...
        if (scene.leftMouseJustDown && d.draggingNodeIdx < 0) {
            ...
            float bestD2 = 4.0f;  // 2-unit miss-distance threshold around sphere center
        ...
        // Apply vertical drag (mouse up = raise)
